...
export default {
  setup() {
    ...
    async function load() {
      try {
        const response = await axios.get('http://localhost:5175/articles');
    ...
    async function save() {
      try {
        const url = form.id
          ? `http://localhost:5175/articles/${form.id}`
          : 'http://localhost:5175/articles/';
    ...
    async function remove(id) {
      try {
        await axios.delete(`http://localhost:5175//articles/${id}`);
